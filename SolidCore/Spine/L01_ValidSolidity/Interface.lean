...
def inFragmentExpr? (fragment : Fragment) : L00_SourceSolidity.Expr -> Bool
...
def inFragmentStmt? (fragment : Fragment) : L00_SourceSolidity.Stmt -> Bool
...
def inFragmentList? (fragment : Fragment) : List L00_SourceSolidity.Stmt -> Bool
  | [] => true
  | stmt :: rest => inFragmentStmt? fragment stmt && inFragmentList? fragment rest

def inFragmentOptional? (fragment : Fragment) : Option L00_SourceSolidity.Stmt -> Bool
  | none => true
  | some stmt => inFragmentStmt? fragment stmt

def inFragmentSwitchCases? (fragment : Fragment) :
    List (SolidCoreYulCore.Word × L00_SourceSolidity.Stmt) -> Bool
  | [] => true
  | (_, stmt) :: rest =>
      inFragmentStmt? fragment stmt && inFragmentSwitchCases? fragment rest

def inFragmentExprList? (fragment : Fragment) : List L00_SourceSolidity.Expr -> Bool
  | [] => true
  | expr :: rest =>
...
structure NameFacts (_stmt : L00_SourceSolidity.Stmt) : Prop where
  namesResolved : True := by trivial
  scopesResolved : True := by trivial

structure TypeFacts (_stmt : L00_SourceSolidity.Stmt) : Prop where
  expressionsTyped : True := by trivial
  lvaluesTyped : True := by trivial
  implicitConversionsChecked : True := by trivial

structure DeclarationFacts (_stmt : L00_SourceSolidity.Stmt) : Prop where
  storageDeclarationsChecked : True := by trivial
  eventsChecked : True := by trivial
  modifiersChecked : True := by trivial

structure ValidSource (fragment : Fragment) (stmt : L00_SourceSolidity.Stmt) :
    Prop where
...
structure Program (fragment : Fragment) where
  ...
  valid : ValidSource fragment stmt
