...
abbrev Word := L01_ValidSolidity.Syntax.Word
abbrev Name := L01_ValidSolidity.Syntax.Name
...
inductive Expr where
  ...
  | env : EnvRead -> Expr
  | storageRead : L01_ValidSolidity.Syntax.StorageId -> List Expr -> Expr
...
inductive Effect where
  | storageWrite : L01_ValidSolidity.Syntax.StorageId -> List Expr -> Expr -> Effect
  | memoryWrite : Expr -> Expr -> Ty -> Effect
  | emitEvent : L01_ValidSolidity.Syntax.EventId -> List Expr -> List Expr -> Effect
...
inductive Completion where
  ...
  | revertError : L01_ValidSolidity.Syntax.ErrorId -> List Expr -> Completion
...
structure Proc where
  id : ProcId
  sourceFunction : Option L01_ValidSolidity.Syntax.FunctionId := none
  params : List Binding := []
