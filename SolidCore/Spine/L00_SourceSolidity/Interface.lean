...
def externalUintGetterTy : Ty :=
  Ty.function [] [Ty.uint 256]
    StateMutability.view Visibility.external_

def externalFunctionPointerGetterSelector : Word :=
  SolidCore.Solidity.Source.ABI.selectorFromSignature "get()"

def externalFunctionPointerGetterCalldata : List Byte :=
  SolidCore.Solidity.Source.wordToBytesBE
    SolidCore.Solidity.Source.selectorBytes
    externalFunctionPointerGetterSelector

def externalFunctionPointerGetterOutput (value : Word) :
    Option (List Byte) :=
  SolidCore.Solidity.Source.abiEncodeValues?
    [SolidCore.Solidity.Source.Ty.uint256]
    [SolidCore.Solidity.Source.Value.word value]

def externalFunctionPointerCallFunction : FunctionDecl :=
  { name := some "callGetter"
...
def externalFunctionPointerCallMatches : Option Bool := do
  let result ← externalFunctionPointerCallResult
  match result with
  | SolidCore.Solidity.Source.CallResult.returned _
      [SolidCore.Solidity.Source.Value.word value] =>
      some (value == 99)
...
def externalPayableUintSetterTy : Ty :=
  Ty.function [Ty.uint 256] []
    StateMutability.payable Visibility.external_

def externalFunctionPointerPayableCallFunction : FunctionDecl :=
  { name := some "callSetter"
    params :=
      [ { name := some "setter", ty := externalPayableUintSetterTy }
      , { name := some "x", ty := Ty.uint 256 } ]
    body :=
      some
        (Stmt.expr
...
def externalFunctionPointerPayableCallMatches : Option Bool := do
  let result ← externalFunctionPointerPayableCallResult
  match result with
...
def externalFunctionPointerTryCatchCatchMatches : Option Bool := do
  let result ←
    externalFunctionPointerTryCatchResult
      SolidCore.Solidity.Source.Context.empty
  match result with
  | SolidCore.Solidity.Source.CallResult.returned _
      [SolidCore.Solidity.Source.Value.word value] =>
...
def highLevelExternalExistenceTargetContract : ContractDecl :=
  { name := "ExistenceTarget"
    items :=
      [ ContractItem.function
          { name := some "notify"
            visibility := some Visibility.external_ }
...
def highLevelExternalExistenceCallerContract : ContractDecl :=
  { name := "ExistenceCaller"
    items :=
      [ ContractItem.function
          { name := some "notifyOrCatch"
            params :=
              [{ name := some "target"
                 ty := Ty.user { segments := ["ExistenceTarget"] } }]
            returns := [{ name := some "out", ty := Ty.uint 256 }]
            body :=
              some
                (Stmt.tryCatchReturns
...
def highLevelExternalExistenceContracts : List ContractDecl :=
  [ highLevelExternalExistenceTargetContract
  , highLevelExternalExistenceCallerContract ]

def highLevelExternalNoReturnMissingCodeCaught : Option Bool := do
  let contract ←
    ContractDecl.toCoreWithBases? highLevelExternalExistenceContracts
      highLevelExternalExistenceCallerContract
  ...
  | SolidCore.Solidity.Source.CallResult.returned _
      [SolidCore.Solidity.Source.Value.word value] =>
      some (SolidCore.Solidity.Source.wordEq value 2)
  | _ => some false

def highLevelExternalNoReturnCodePresentSucceeds : Option Bool := do
  let contract ←
    ContractDecl.toCoreWithBases? highLevelExternalExistenceContracts
      highLevelExternalExistenceCallerContract
  let function ← contract.findFunctionByName? "notifyOrCatch"
...
def highLevelExternalReturnNoCodeUsesReturndata : Option Bool := do
  let contract ←
    ContractDecl.toCoreWithBases? highLevelExternalExistenceContracts
      highLevelExternalExistenceCallerContract
  let function ← contract.findFunctionByName? "readNoCodeReturn"
  let callData ← externalCalldata? "get()" [] []
  let output ←
    SolidCore.Solidity.Source.ABI.encodeValues?
      [SolidCore.Solidity.Source.Ty.uint256]
  ...
  | SolidCore.Solidity.Source.CallResult.returned _
      [SolidCore.Solidity.Source.Value.word value] =>
      some (SolidCore.Solidity.Source.wordEq value 5)
  | _ => some false
...
def highLevelExternalViewTargetContract : ContractDecl :=
  { name := "ViewTarget"
    items :=
      [ ContractItem.function
          { name := some "get"
...
def highLevelExternalViewCallerContract : ContractDecl :=
  { name := "ViewCaller"
    items :=
      [ ContractItem.function
          { name := some "readView"
            params :=
              [ { name := some "target"
                  ty := Ty.user { segments := ["ViewTarget"] } } ]
            returns := [{ name := some "out", ty := Ty.uint 256 }]
            body :=
              some
                (Stmt.returnValues
...
def highLevelExternalViewBadValueCallerContract : ContractDecl :=
  { name := "ViewBadValueCaller"
    items :=
      [ ContractItem.function
...
def highLevelExternalViewContracts : List ContractDecl :=
  [highLevelExternalViewTargetContract, highLevelExternalViewCallerContract]

def highLevelExternalViewStaticcallMatches : Option Bool := do
  let contract ←
    ContractDecl.toCoreWithBases?
      highLevelExternalViewContracts highLevelExternalViewCallerContract
  let function ← contract.findFunctionByName? "readView"
  let callData ← externalCalldata? "get()" [] []
  let output ←
    SolidCore.Solidity.Source.ABI.encodeValues?
      [SolidCore.Solidity.Source.Ty.uint256]
...
def highLevelExternalPureGasStaticcallMatches : Option Bool := do
  let contract ←
    ContractDecl.toCoreWithBases?
      highLevelExternalViewContracts highLevelExternalViewCallerContract
  let function ← contract.findFunctionByName? "readPureWithGas"
  let callData ← externalCalldata? "pureGet()" [] []
  let output ←
    SolidCore.Solidity.Source.ABI.encodeValues?
      [SolidCore.Solidity.Source.Ty.uint256]
...
def highLevelExternalGetterStaticcallMatches : Option Bool := do
  let contract ←
    ContractDecl.toCoreWithBases?
      highLevelExternalViewContracts highLevelExternalViewCallerContract
  let function ← contract.findFunctionByName? "readGetter"
...
def highLevelExternalViewValueOptionRejected : Option Bool := do
  match
    ContractDecl.toCoreWithBases?
...
def publicStringArrayGetterContract : ContractDecl :=
  { name := "PublicStringArray"
    items :=
      [ ContractItem.stateVar
          { name := "names"
            ty := Ty.array Ty.string none
            visibility := some Visibility.public_ } ] }

def publicStringArrayGetterState : CoreState :=
  let elementSlot :=
...
def publicStringArrayGetterMatches : Option Bool := do
  let result ←
    ContractDecl.call? 24 publicStringArrayGetterContract
      (SolidCore.Solidity.Source.CallTarget.name "names")
      publicStringArrayGetterState
...
def publicStringArrayGetterCalldataMatches : Option Bool := do
  let contract ← ContractDecl.toCore? publicStringArrayGetterContract
  let function ← contract.findFunctionByName? "names"
  let calldata ←
    SolidCore.Solidity.Source.ABI.calldataFor? function
      [SolidCore.Solidity.Source.Value.word 0]
  let result ←
    SolidCore.Solidity.Source.ABI.Contract.callCalldata?
      24 contract publicStringArrayGetterState calldata
  let expected ←
    SolidCore.Solidity.Source.ABI.encodeValues?
      [SolidCore.Solidity.Source.Ty.bytesCalldata]
...
def publicFixedBytesArrayGetterContract : ContractDecl :=
  { name := "PublicFixedBytesArray"
    items :=
      [ ContractItem.stateVar
          { name := "fixedBlobs"
            ty := Ty.array Ty.bytes (some 2)
...
def publicFixedBytesArrayGetterState : CoreState :=
  let elementSlot :=
...
def publicFixedBytesArrayGetterMatches : Option Bool := do
  let result ←
    ContractDecl.call? 24 publicFixedBytesArrayGetterContract
      (SolidCore.Solidity.Source.CallTarget.name "fixedBlobs")
      publicFixedBytesArrayGetterState
      [SolidCore.Solidity.Source.Value.word 1]
  match result with
  | SolidCore.Solidity.Source.CallResult.returned _
      [SolidCore.Solidity.Source.Value.bytes bytes] =>
...
def publicFixedBytesArrayGetterCalldataMatches : Option Bool := do
  let contract ← ContractDecl.toCore? publicFixedBytesArrayGetterContract
  let function ← contract.findFunctionByName? "fixedBlobs"
  let calldata ←
    SolidCore.Solidity.Source.ABI.calldataFor? function
      [SolidCore.Solidity.Source.Value.word 1]
  let result ←
    SolidCore.Solidity.Source.ABI.Contract.callCalldata?
      24 contract publicFixedBytesArrayGetterState calldata
  let expected ←
    SolidCore.Solidity.Source.ABI.encodeValues?
      [SolidCore.Solidity.Source.Ty.bytesCalldata]
      [SolidCore.Solidity.Source.Value.bytes [7, 8, 9]]
  some (result.success && result.output == expected)

def publicFixedBytesArrayGetterOutOfBounds : Option Bool := do
  let result ←
    ContractDecl.call? 24 publicFixedBytesArrayGetterContract
      (SolidCore.Solidity.Source.CallTarget.name "fixedBlobs")
      publicFixedBytesArrayGetterState
