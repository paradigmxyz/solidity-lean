...
/-!
Repo-owned pure Keccak-256.

The pinned `danrobinson/EVMYulLean @ 3c5c44a6` package supplies Keccak only as an
FFI symbol (`@[extern "keccak256"] opaque keccak256 …` in `EvmYul/FFI/ffi.lean`;
its `EvmYul/SpongeHash/Keccak256.lean` is an empty stub). An opaque/extern hash
does not reduce and cannot back this repo's total, purely computational fuel
interpreter, so — per the roadmap's Phase 1 fallback — this repo keeps its own
pure Lean implementation, named explicitly under `SolidCore.Solidity.Source`
rather than masquerading as an `EvmYul.*` upstream module. Byte-parity against
the pinned FFI hash is asserted separately (see `SolidCore/Solidity/KeccakParity.lean`).

This is a verbatim relocation of the former `EvmYul/SpongeHash/Keccak256.lean`
implementation; its bytes are unchanged, so corpus expectations are preserved.
-/
