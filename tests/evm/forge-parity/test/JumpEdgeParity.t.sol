...
contract JumpEdgeParityTest {
    JumpEdgeVm internal constant vm = JumpEdgeVm(address(uint160(uint256(keccak256("hevm cheat code")))));

    address internal constant TARGET = 0x1000000000000000000000000000000000000001;
    uint256 internal constant GAS_LIMIT = 1_000_000;

    function testJumpIntoPushDataJumpdestByteIsInvalidParity() public {
        bytes memory code = hex"615b00600156";
        runCaseSkippingGas("jump-into-pushdata-jumpdest-byte-invalid", code);
    }

    function testJumpiTrueIntoPushDataJumpdestByteIsInvalidParity() public {
        bytes memory code = hex"615b006001600157";
        runCaseSkippingGas("jumpi-true-into-pushdata-jumpdest-byte-invalid", code);
    }

    function testJumpiFalseIgnoresPushDataJumpdestByteParity() public {
        bytes memory code = hex"615b005f600157602a5f5260205ff3";
        runCase("jumpi-false-ignores-pushdata-jumpdest-byte", code);
    }

    function testJumpOverPushDataToRealJumpdestParity() public {
        bytes memory code = hex"615b006006565b602a5f5260205ff3";
        runCase("jump-over-pushdata-to-real-jumpdest", code);
    }

    function testTruncatedPushAtEofStopsParity() public {
        bytes memory code = hex"6112";
        runCase("truncated-push-at-eof-stops", code);
    }

    function testJumpIntoTruncatedPushDataIsInvalidParity() public {
        bytes memory code = hex"600456615b";
        runCaseSkippingGas("jump-into-truncated-pushdata-invalid", code);
    }

    function testPcAfterPush32Parity() public {
        bytes memory code = hex"7f0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20585f5260205ff3";
        runCase("pc-after-push32", code);
    }
    ...
    function runCaseWithGasPolicy(string memory name, bytes memory code, bool skipGas) internal {
        ...
        JumpEdgeVm.Gas memory gas = vm.lastCallGas();
