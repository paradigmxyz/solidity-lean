...
contract ExpGasParityTest {
    ExpGasVm internal constant vm = ExpGasVm(address(uint160(uint256(keccak256("hevm cheat code")))));

    address internal constant TARGET = 0x1000000000000000000000000000000000000001;
    uint256 internal constant GAS_LIMIT = 1_000_000;

    function testExpZeroExponentGasParity() public {
        bytes memory code = hex"5f60070a5f5260205ff3";
        runCase("exp-zero-exponent-gas", code);
    }

    function testExpOneByteExponentGasParity() public {
        bytes memory code = hex"60ff60010a5f5260205ff3";
        runCase("exp-one-byte-exponent-gas", code);
    }

    function testExpTwoByteExponentGasParity() public {
        bytes memory code = hex"61010060020a5f5260205ff3";
        runCase("exp-two-byte-exponent-gas", code);
    }

    function testExpThirtyTwoByteExponentGasParity() public {
        bytes memory code = hex"7fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff60010a5f5260205ff3";
        runCase("exp-thirty-two-byte-exponent-gas", code);
    }
    ...
    function runCase(string memory name, bytes memory code) internal {
        ...
        ExpGasVm.Gas memory gas = vm.lastCallGas();
