...
contract StackEdgeParityTest {
    StackEdgeVm internal constant vm = StackEdgeVm(address(uint160(uint256(keccak256("hevm cheat code")))));

    address internal constant TARGET = 0x1000000000000000000000000000000000000001;
    uint256 internal constant GAS_LIMIT = 1_000_000;

    function testDup16UnderflowParity() public {
        bytes memory code = hex"5f8f";
        runCaseSkippingGas("dup16-underflow", code);
    }

    function testSwap16BoundaryUnderflowParity() public {
        bytes memory code = new bytes(17);
        for (uint256 i = 0; i < 16; i++) {
            code[i] = 0x5f;
        }
        code[16] = 0x9f;
        runCaseSkippingGas("swap16-boundary-underflow", code);
    }

    function testUndefinedOpcodeParity() public {
        bytes memory code = hex"0c";
        runCaseSkippingGas("undefined-opcode", code);
    }
    ...
    function runCaseSkippingGas(string memory name, bytes memory code) internal {
        ...
        StackEdgeVm.Gas memory gas = vm.lastCallGas();

        string[] memory command = new string[](44);
        command[0] = "python3";
