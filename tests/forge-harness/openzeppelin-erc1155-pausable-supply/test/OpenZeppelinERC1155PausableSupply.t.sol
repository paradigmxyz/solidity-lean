...
import {
    OpenZeppelinERC1155PausableSupplyHarness,
    ...
    OpenZeppelinERC1155PausableSupplySpender
} from "../src/OpenZeppelinERC1155PausableSupply.sol";

contract OpenZeppelinERC1155PausableSupplyForgeTest {
    ...
    function testMetadataInterfacesAndInitialSupply() public {
        OpenZeppelinERC1155PausableSupplyHarness token =
            new OpenZeppelinERC1155PausableSupplyHarness();

        require(
            keccak256(bytes(token.uri(7))) ==
                keccak256(bytes("ipfs://pausable-supply.example/{id}.json")),
            "uri"
        );
        require(
            token.supportsInterface(
                type(OpenZeppelinERC1155PausableSupplyIERC1155).interfaceId
            ),
            "core id"
        );
        require(
    ...
    function testMintBatchTransferAndBurnSupply() public {
        OpenZeppelinERC1155PausableSupplyHarness token =
            new OpenZeppelinERC1155PausableSupplyHarness();
        OpenZeppelinERC1155PausableSupplySpender spender =
            new OpenZeppelinERC1155PausableSupplySpender();
        ERC1155PausableSupplyApprovalActor actor = new ERC1155PausableSupplyApprovalActor();
    ...
    function testPauseBlocksMintTransferBurnAndRollsBackSupply() public {
        OpenZeppelinERC1155PausableSupplyHarness token =
            new OpenZeppelinERC1155PausableSupplyHarness();
        ERC1155PausableSupplyApprovalActor actor = new ERC1155PausableSupplyApprovalActor();
        address alice = address(actor);
        ...
        actor.setApproval(token, address(this));
        token.pause();
        require(token.paused(), "paused");

        try token.mint(alice, 2, 5) {
        ...
        require(token.totalSupply() == 10, "mint rollback all");
        require(token.totalSupply(2) == 0, "mint rollback id");

        try token.safeTransferFrom(alice, address(0xb0b), 1, 3, "") {
        ...
        require(token.balanceOf(alice, 1) == 10, "transfer rollback balance");
        require(token.totalSupply(1) == 10, "transfer rollback supply");

        try token.burn(alice, 1, 4) {
        ...
        require(token.balanceOf(alice, 1) == 10, "burn rollback balance");
        require(token.totalSupply() == 10, "burn rollback all");

        token.unpause();
        require(!token.paused(), "unpaused");
        token.burn(alice, 1, 4);
        require(token.balanceOf(alice, 1) == 6, "burn after unpause");
        require(token.totalSupply() == 6, "supply after unpause");
    }

}

contract ERC1155PausableSupplyApprovalActor {
