...
| Internal-function recursion / call-nesting deeper than the inline budget | **Fixed for value + memory-ref + storage-ref-parameter callees** (function-boundary refactor + reference-signature extension, branch `refactor/ref-signatures`); residual: storage-ref-RETURN / calldata / function-typed callees | Internal-linkage calls whose resolved callee has a value, `memory`-ref, or `storage`-ref-parameter signature (returns value or `memory`-ref) now elaborate to `Stmt.internalCall` nodes resolved against `Contract.table` and execute as framed in-monad calls: `memory` args flow as pointers (callee mutations alias back), `storage` args flow as `Value.storageRef`/`storagePathRef` runtime values resolved against the shared `state` (solc's model), value args by value; recursion / arbitrarily deep nesting through these signatures elaborate and run. The heavy OZ `using`-for storage-ref library corpus (enumerable-set/map, checkpoints, bitmaps, counters, arrays, timers, double-ended-queue) and the memory-struct-heavy compound-exponential lane are green; the five `reference-*`/`function-type-locations` sentinels pass. Table keys are disambiguated by struct/enum/user identity (`internalTableKey?` appends a structural `repr`) to prevent overload misdispatch. **Residual gap**: callees that RETURN a `storage` pointer, or take/return `calldata` references or function-typed values, still inline-splice under `defaultInternalCallInlineFuel = 64`, so *recursion through those* is still silently rejected (`toCoreContract?` = none). Storage-ref-return needs the wrapper-caller capture wiring finished (callee-side + interpreter groundwork already in place); function-typed needs an internal-function-pointer `Value`. Tracked in `docs/function-boundary-refactor-plan.md` + the reference-signature DECISIONS entry. |
