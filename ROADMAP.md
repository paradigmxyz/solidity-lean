...
| Internal-function recursion / call-nesting deeper than the inline budget | In progress — plan exists (`docs/function-boundary-refactor-plan.md`); **pinned by the `recursion-gap` corpus lane (stage 0)** | Internal-linkage calls (internal/free/library/`using`/`super`) are **inlined at elaboration** with `defaultInternalCallInlineFuel = 64` (`Interface.lean:10111`); a residual call at inline-fuel 0 makes `toCoreContract?` return `none`, so recursive functions and static call-nesting deeper than 64 are **silently rejected** though solc accepts them — an *acceptance* gap (no differential lane catches it because nothing observable runs). The `recursion-gap` paired lane now pins this: solc/Forge run `factorial(5)=120`, `sumTo(70)=2485`, `deepChain()=70`; the Lean witnesses assert `ownCall?` returns `none` (rejected today) and FLIP to those values at stage 5. Fix = preserve the function boundary (a `Stmt.internalCall` node + function table + framed in-monad calls, fuel-bounded like loops); this also gives the future Sc (calling-convention) lowering layer its source IR. |
