...
- **Semantic gap fixes** (balance accounting, `gasleft` as a resource query):
  both now **fixed (2026-07-06)** — see the gap registry below. (Rational
  constant folding: the A1 over-reject is also **fixed** — the audit found no
  unsoundness.)
...
| Gap | Status | Notes |
| --- | --- | --- |
| Intra-frame balance accounting | **Fixed (2026-07-06)** | Self balance is now dynamic: `State.selfBalance` is re-based at each external entry (`FunctionDef.evalBodyEntry`) to `balanceAt accountBalances self` credited with `msg.value` (before body execution, incl. constructor entry), debited by successful value-carrying calls/`send`/`transfer` and value creates (centralized in `Runtime.recordExternalInteraction`; failed send → no debit, EVM-refund semantics), and read by `address(this).balance` / `selfbalance`. Other addresses stay environment facts (static `accountBalances` oracle). Consistent with the Yul `OpenWorld` model within a frame (credit-before-body, debit-on-call, self=codeOwner); cross-call balance stays environment-supplied (open-world), `postWorld` still ignored (checkpoint-1). Pinned by the `balance-accounting` Forge lane (constructor credit, function credit, successful-send debit, failed-send no-debit) + witnesses; the one hand-tuned corpus oracle (`base-constructor-runtime-args` `BalanceArg`, which both received value and read its own balance) was corrected from the post-credit `[(0xcafe,10)]` to the pre-credit fact `[]`. See `docs/DECISIONS.md`. |
...
| `gasleft` as resource query | **Fixed (2026-07-06)** | `EnvWord.gasleft` now emits `Query.resource .gas` (the shared alphabet's reserved arm) via `emitGasleft`, resuming on the answered word, instead of reading the ambient constant directly. Every answerer supplies the ambient `context.gasleft` — `contextAnswer`/`SolI.run` and `SolI.runFromContext`/`foldExpr` gain an explicit `resource .gas` arm returning `wordToU256 context.gasleft`; the fail-closed corpus responder `SolI.runWith` gets an explicit `resource` arm (answered ambiently, NOT matched against call/create rows), taking the canonical `0` default in the context-free fold (behaviour-preserving: 0 corpus `gasleft` users). `buildCallRequest.requestedGas` keeps its fill rule. Value unchanged by construction; the query now appears in the transcript. Pinned by `SolidCore.Witness.GasleftResource` (transcript contains `resource .gas`, folded value = ambient word). See `docs/DECISIONS.md`. |
...
| Open-world `postWorld` adoption (arbitrary environment changes / reentrancy) | **Fixed (2026-07-07; branch `openworld/postworld`)** | The external boundary is now shape-identical to Yul's `installYulShared`. Every `Query.external` carries a real `OpenWorld` snapshot (`snapshotWorld`, mirror of `ofYulShared`); on resume the answered `CallResponse.postWorld`/`CreateResponse.postWorld` is adopted WHOLESALE (`adoptWorld`): self-account storage/transient/balance/nonce land on the existing `State` fields (ordinary slot-keyed writes — no representation change), the answered world is retained verbatim in `State.envWorld?` as the live other-accounts/substate/created-accounts view, and env-fact reads route through `State.env*` (adopted world first, Context maps as entry seeds). The **round-trip law** `snapshotWorld context (adoptWorld w context s) = w` is proved for every `w` (`SolidCore/Solidity/AdoptionLaws.lean`, mirror of `ofYulShared_installYulShared`), plus `adoptWorld_echo_noop`. Responder rows gained an optional `PostDelta` (model-level slot-keyed writes); the A2 outgoing-call debit is **folded into the responder's echo answer** and arrives through adoption (removed from `recordExternalInteraction`). The earlier "fail-closed re-projection" resolution of the self-storage rows above (`docs/DECISIONS.md:310-319`) is **superseded**: typed reads of non-canonical adopted words are TOTAL and solc-faithful (Stage 0), so no layout-encoding inverse or environment well-formedness hypothesis is needed. Pinned by `storage-dirty-words` (Stage 0 total reads) and `reentrancy-adoption` (five Forge-traced lanes: reentrant storage write, ill-encoded word read through adoption, balance-refunding callee where adoption—not the debit—is authoritative, transient mutation, create-with-reentry). See `docs/openworld-postworld-plan.md` + `docs/DECISIONS.md`. |
| Gas metering | Out of scope by design | Solidus handles real gas in its gasful refinement conjunct. |
| Internal-function recursion / call-nesting deeper than the inline budget | **Fixed — all signatures** (function-boundary refactor + boundary-completion arc, branch `refactor/ref-signatures`) | Every internal-linkage callee — value, `memory`-ref, `storage`-ref (params AND returns, including mapping-typed storage params), calldata-ref, and function-typed signatures — elaborates to `Stmt.internalCall`/`Stmt.internalCallPtr` nodes resolved against `Contract.table` and executes as framed in-monad calls (locals replaced, state shared, fuel-bounded like loops). The inline-splice machinery is DELETED (stage E): no callee splices, and recursion/deep nesting through ANY signature elaborates and runs. Internal function pointers are solc via-IR dispatch IDs (storable 8-byte state variables, 64-bit-mask read, call-time `Panic(0x51)` on invalid IDs — adoption-planted dirty words behave exactly as solc). Pinned by the paired lanes `recursion-gap` (value), `recursion-ref-signatures` (storage-ref linked-list walk over a mapping; memory-ref recursion + aliasing), `internal-fn-pointers` (pointer recursion; uninitialized-pointer call → Panic 0x51), and `calldata-ref-internal` (calldata recursion). `defaultInternalCallInlineFuel` survives only as a nested-call-argument hoisting bound (syntactic, per-expression); recursion depth is bounded only by the interpreter statement fuel (`outOfFuel`). See the boundary-completion DECISIONS entries + `docs/refs-completion-solc-research.md`. |
