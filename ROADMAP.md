...
| Internal-function recursion / call-nesting deeper than the inline budget | **Fixed for value-signature callees** (function-boundary refactor stages 2-3, branch `refactor/function-boundary`); residual: ref-signature callees | Internal-linkage calls whose resolved callee has a pure stack-value signature (all params/returns without a data location) now elaborate to `Stmt.internalCall` nodes resolved against `Contract.table` and execute as framed in-monad calls (locals replaced, state shared, fuel-bounded like loops) — recursion and arbitrarily deep call nesting elaborate and run. The `recursion-gap` lane's witnesses assert the concrete Forge values (`factorial(5)=120`, `sumTo(70)=2485`, `deepChain()=70`) and pass. **Residual gap**: callees with storage-ref / memory-ref / function-typed params or returns still inline-splice under `defaultInternalCallInlineFuel = 64`, so *recursion through such signatures* is still silently rejected (`toCoreContract?` = none). Extending the boundary to ref signatures (storage pointers ARE runtime `Value`s — `Value.storageRef` — but elaboration lowers storage-ref args as `storageAlias*` statements, not value expressions) is the remaining work, tracked in `docs/function-boundary-refactor-plan.md` + the stage 3/4 DECISIONS entries. |
