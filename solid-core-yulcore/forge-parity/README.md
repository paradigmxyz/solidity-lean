...
```sh
python3 scripts/evm_parity.py forge
```
...
python3 scripts/foundry_corpus.py scan --suite local-forge-parity
```
...
callee-perspective `vm.lastCallGas()`, and calls `scripts/evm_parity.py check`
through FFI. The checker runs `.lake/build/bin/evm_parity` when available, or
falls back to `lake exe evm_parity`.

Useful fixture knobs exposed by the Lean CLI:
...
Current fixtures cover return, revert, `GAS`, cold `SLOAD`, cold `SSTORE`,
...
multi-contract fixtures install separate caller, worker, and delegate library
...
Dynamic deployment fixtures now cover `CREATE` and `CREATE2` by passing the
