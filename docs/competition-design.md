...
entrants submit Forge tests that demonstrate a divergence between **solidity-lean**
...
tables, lines 34–69); the harness Forge-vs-solidity-lean flow
...
`docs/solidity-feature-coverage.md`, `docs/solidity-lean-solc-deep-comparison.md`
...
solidity-lean is a source-level semantics with an **open-world interaction model**.
...
solc+EVM produce observable O, but solidity-lean produces O′ ≠ O (or solidity-lean cannot run
the program at all)."* The contest exists because solidity-lean is validated **only** by
...
- **COVERAGE-gap lane (lane C).** A legitimate, in-scope Solidity 0.8.35 feature
  that solc accepts + runs, but solidity-lean **fails closed** on: the importer
  `fail()`s, or type-check/elaboration rejects, or execution cannot proceed.
  Must NOT be an intentional exclusion (see §1).
- **SOUNDNESS-gap lane (lane S).** solidity-lean **runs** the program to completion but
  ...
  ground is the G-register (G1–G22) in `docs/solidity-lean-solc-deep-comparison.md`.

Both lanes require the claimed behavior to be **real on solc+EVM** — the Forge
test must pass against pinned solc 0.8.35 and Foundry-EVM before solidity-lean is ever
...
every place solidity-lean **intentionally** does not model 0.8.35, so that a rejection
...
observable that solidity-lean does not intend to model faithfully**. Presence is not a
...
| SEM-CODE | An observable that depends on **real compiled bytecode** — `type(C).creationCode.length`, `keccak256(type(C).creationCode)`, `runtimeCode`, `address(c).code`, `.codehash` of created code | flag when a `bytes`/`bytes32`/`uint` derived from `creationCode`/`runtimeCode`/`.code`/`.codehash` of a submission-created contract flows to an assertion or observed return | solidity-lean's initCode is `len‖name‖args` (source-canonical), not EVM bytecode (`ROADMAP.md:468`). |
| SEM-ADDR | An observable that is a **create2 predicted address** (or any address that depends on init bytecode) | flag when the address of a `new C{salt:…}()` result, or a hand-computed `keccak(0xff‖…)` create2 address, flows to an assertion/observed return | create2 address depends on `keccak(initcode)`; solidity-lean lacks it (G22). Non-salted `new` addresses ARE modeled (`CreateResponse.address`) and are in scope. |
| SEM-CLOSEDGAS | Closed-world gas metering / OOG as an observable | any assertion that a call reverts *specifically due to out-of-gas*, or that distinguishes success/revert by a gas stipend (`.send`/`.transfer` 2300-stipend-driven behavior where the difference is the stipend, not logic) | OOS by the same gas decision; solidity-lean does not meter the 2300 stipend. |
...
**before** solidity-lean import. It consumes the same solc JSON AST the importer
consumes (pinned solc 0.8.35, `--ast-compact-json`), so it sees exactly what
solidity-lean would see.
...
- `PASS { register_version }` — no exclusion fired; the submission proceeds to
  solidity-lean import + adjudication (§3).

**How it composes with the importer's fail-closed guard.** solidity-lean's importer
...
So the pipeline turns solidity-lean's existing fail-closed partition into the contest's
...
submitted contracts under solidity-lean and resolve each external call to the actual
...
1. **You cannot validate a specific multi-contract interaction's value.** The
   ...
   divergence, if any, lives entirely inside B, which solidity-lean never ran.
...
gas() to distinguish"): we run **all** contracts with solidity-lean (so composed values
...
slots/getters the Forge test observes, because solidity-lean models storage layout
...
1. REAL-BEHAVIOR CHECK  (the claimed behavior must be real)
   ...
   1a. solc REJECTS the program (compile error), and the submission's lane is S
       with claim "solidity-lean wrongly ACCEPTS":
...
3. RUN SOLIDITY-LEAN
   ...
   (a) solidity-lean FAILS CLOSED  (importer fail() / typecheck sentinel / elaboration
   ...
   (b) mode = OVER_ACCEPT (step 1a): solidity-lean imports+typechecks+runs a program
       solc REJECTED  ⇒  **SOUNDNESS_GAP (lane S, over-accept).**  [terminal]

   (c) solidity-lean RUNS to completion:
         - Compute solidity-lean observable (§3.4). Compute solc+EVM observable from the
           Forge run.
   ...
   (d) solidity-lean wrongly REJECTS a program solc accepts+ran (a fail-closed in 3a
       ...
       sub-kind OVER_REJECT — solidity-lean fails closed on an in-scope, solc-accepted
...
believes solidity-lean does wrong, and the solc value they measured. The harness does
...
Forge-vs-solidity-lean machinery:
...
4. **Compare** — the driver compares the solidity-lean `#eval` observable against the
...
- **(a) Excluded-feature smuggling** — defeated by §2's whole-submission scan:
  ...
  in a transitive callee is caught before solidity-lean runs.
- **(b) Duplicate / known-gap resubmission** — every submission is deduped against
  **two** registries: (i) the **exclusion register** (§1) — an OOS hit is not a
  ...
  (`docs/solidity-lean-solc-deep-comparison.md` G1–G22, plus any W/H items). A submission
...
- lane S: `(observable_component, minimal_feature, solc_vs_solidity_lean_delta_shape)` —
...
- **State-observable scope.** solidity-lean models storage but not memory; the contest
...
1. **Two lanes**, adjudicated by the §4 tree: COVERAGE_GAP (solidity-lean fails closed
   on an in-scope, solc-accepted feature) and SOUNDNESS_GAP (solidity-lean runs but the
   observable differs, incl. over-accept/over-reject).
2. **Reject gate** = a whole-submission AST scan (`reject_gate.py`) over solc's AST
   of every source, run before solidity-lean import, reusing the importer's
   `EXCLUDED_NODE_TYPES` so the register and importer cannot drift; it **classifies**
   solidity-lean's existing fail-closed partition into OUT_OF_SCOPE vs coverage-gap.
...
- **Inconclusive solidity-lean failures** (timeout / resource exhaustion / poisoned
...
slots. The entry call runs against the **post-construction** state: solidity-lean runs
...
Validated end-to-end against the built solidity-lean (`ctor_storage` sample +
`run_real_storage_selftest`).

**Gap-testing without leaving bugs in solidity-lean.** The soundness detector is now
...
a genuine solidity-lean execution while solidity-lean stays bug-free. When a real coverage
...
diffs solidity-lean's `#eval` against THAT. `declared_observable` is kept only as a
...
holds these; the solidity-lean `#eval` threads them into the Context/BlockEnv via the
...
their literal effect is mirrored into the solidity-lean env; everything else on
...
+ Foundry + the built solidity-lean in this worktree: the original four stay green
(`oos_gasleft`→REJECTED_OOS, `no_divergence`→NO_DIVERGENCE, plus the two
synthetic solidity-lean-simulated `coverage_gap`→COVERAGE_GAP /
